# Croptesting

## 5.6

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 5.6/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 5.6/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 5.6/php_xdebug/Dockerfile . && docker run croptesting
```
## 7.1

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.1/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.1/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.1/php_xdebug/Dockerfile . && docker run croptesting
```
## 7.2

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.2/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.2/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.2/php_xdebug/Dockerfile . && docker run croptesting
```
## 7.3

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.3/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.3/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.3/php_xdebug/Dockerfile . && docker run croptesting
```
## 7.4

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.4/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.4/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 7.4/php_xdebug/Dockerfile . && docker run croptesting
```
## 8.0

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.0/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.0/php-alpine/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.0/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.0/php_supervisor/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.0/php_xdebug/Dockerfile . && docker run croptesting
```
## 8.1

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.1/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.1/php-alpine/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.1/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.1/php_supervisor/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.1/php_xdebug/Dockerfile . && docker run croptesting
```
## 8.2

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.2/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.2/php-alpine/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.2/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.2/php_supervisor/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.2/php_xdebug/Dockerfile . && docker run croptesting
```
## 8.3

```bash
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.3/php/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.3/php_blackfire/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.3/php_supervisor/Dockerfile . && docker run croptesting
DOCKER_BUILDKIT=1 docker build -t croptesting -f 8.3/php_xdebug/Dockerfile . && docker run croptesting
```